...
class TestStringClient:
    ...
    def test_map_identifiers(self):
        from src.clients.string_db import map_identifiers
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = [
            {"queryItem": "A0B9K2", "stringId": "2157.A0B9K2"},
        ]
        with patch("src.clients.string_db.post_with_retry", return_value=mock_resp):
            result = map_identifiers(["A0B9K2"])
        assert result["A0B9K2"] == "2157.A0B9K2"

    def test_fetch_interactions(self):
        from src.clients.string_db import fetch_interactions
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = [
            {
                "stringId_A": "2157.A0B9K2",
                "stringId_B": "2157.A0B5T7",
                "preferredName_B": "atpB",
                "score": 750,
            }
        ]
        uniprot_map = {"A0B9K2": "2157.A0B9K2"}
        with patch("src.clients.string_db.post_with_retry", return_value=mock_resp):
            records = fetch_interactions(["2157.A0B9K2"], uniprot_map=uniprot_map)
        assert len(records) == 1
        assert records[0].enrichment_type == EnrichmentType.INTERACTION_PARTNER
        assert records[0].uniprot_accession == "A0B9K2"
        assert records[0].score == 0.75
        assert records[0].label == "atpB"

    def test_fetch_enrichment(self):
        from src.clients.string_db import fetch_enrichment
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = [
            {
        ...
        with patch("src.clients.string_db.post_with_retry", return_value=mock_resp):
            records = fetch_enrichment(["2157.A0B9K2"])
        assert len(records) == 1
        assert records[0].enrichment_type == EnrichmentType.FUNCTIONAL_ENRICHMENT
        assert records[0].value == "GO:0006754"
        assert records[0].label == "ATP biosynthetic process"
        assert records[0].score == 0.001
        assert records[0].uniprot_accession == "SET_LEVEL"
    ...
    def test_empty_string_response(self):
        ...
        mock_resp.json.return_value = []
        with patch("src.clients.string_db.post_with_retry", return_value=mock_resp):
            records = fetch_interactions(["2157.X00000"], uniprot_map={})
        assert records == []
